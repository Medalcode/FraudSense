...
if page == "📊 Overview":
    ...
    with col_c:
        ...
        fig_dev.update_layout(margin=dict(t=20), showlegend=False, coloraxis_showscale=False)
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    country_stats["tasa_fraude"] = (country_stats["fraudes"] / country_stats["total"] * 100).round(
        2
    )
    ...
    st.dataframe(
        country_stats.rename(
            columns={
                ...
                "total": "Total",
                "fraudes": "Fraudes",
                "tasa_fraude": "Tasa Fraude (%)",
                "monto_prom": "Monto Prom. (CLP)",
            }
        ).round(0),
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    fig_line.add_vrect(
        ...
        opacity=0.08,
        line_width=0,
        annotation_text="Horas de Alto Riesgo",
    ...
    with col2:
        ...
        m1, m2 = st.columns(2)
        m1.metric(
            "Tasa Fraude Nocturna",
            f"{night['is_fraud'].mean() * 100:.2f}%",
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    if not model_available:
        st.warning(
            "⚠️ Modelo no entrenado aún. Ejecuta `python src/train_model.py` para activar el evaluador."
        )
    ...
    with col_form:
        ...
        amount = st.number_input(
            "💰 Monto (CLP)", min_value=100, max_value=50_000_000, value=350_000, step=10_000
        )
        country = st.selectbox(
            "🌍 País", ["CL", "AR", "RU", "NG", "CN", "BR", "PE", "MX", "US", "DE", "VN", "UA"]
        )
        hour = st.slider("⏰ Hora del día", 0, 23, 14)
        device_type = st.selectbox(
            "📱 Dispositivo", ["Android", "iOS", "Web", "Windows", "Unknown"]
        )
    ...
    with col_result:
        ...
        if evaluate:
            ...
            if model_available and MODEL_READY:
                ...
                css_class = {"ALTO": "alert-alto", "MEDIO": "alert-medio", "BAJO": "alert-bajo"}[
                    level
                ]
                ...
                st.info(
                    f"📎 Evaluación heurística (sin modelo ML): **{score_h:.1%}** — Riesgo **{level_h}**"
                )
    ...
    st.dataframe(
        filtered[
            ...
            columns={
                ...
                "high_risk_merchant": "Alto Riesgo",
            }
        ),
        use_container_width=True,
        height=500,
