...
def generate_transactions(
    n_total: int = DATASET_SIZE, fraud_rate: float = FRAUD_RATE, seed: int = RANDOM_STATE
) -> pd.DataFrame:
    """
    ...
    legit = pd.DataFrame(
        {
            ...
            "device_type": rng.choice(DEVICE_TYPES[:4], size=n_legit, p=[0.35, 0.35, 0.25, 0.05]),
    ...
    fraud = pd.DataFrame(
        {
            ...
            "device_type": rng.choice(DEVICE_TYPES, size=n_fraud, p=[0.25, 0.20, 0.15, 0.10, 0.30]),
            "failed_attempts": rng.choice(
                [3, 4, 5, 6, 7, 8], size=n_fraud, p=[0.20, 0.20, 0.20, 0.15, 0.15, 0.10]
            ),
